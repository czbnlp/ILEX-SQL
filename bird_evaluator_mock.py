...
class BIRDEvaluator:
    ...
    def __init__(self, 
                 ...
                 db_root: str = "/Users/chuzhibo/Desktop/workspace_sql/data/dev_databases",
                 timeout: int = 30):
        """
        初始化评估器
        ...
        try:
            ...
            for table in tables:
                ...
                for col in columns:
                    # 处理不同数量的列
                    if len(col) >= 6:
                        col_name, col_type, not_null, default_val, is_pk, *extra = col
                    elif len(col) == 5:
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="BIRD数据集评估器（模拟版）")
    parser.add_argument("--split", type=str, default="dev", choices=["dev", "test", "train"],
                       help="数据集分割")
    parser.add_argument("--max-questions", type=int, default=None,
                       help="最大评估问题数量（用于测试）")
    parser.add_argument("--output", type=str, default="bird_evaluation_results_mock.json",
                       help="输出文件路径")
    ...
    try:
        # 创建评估器
        evaluator = BIRDEvaluator(timeout=args.timeout)
